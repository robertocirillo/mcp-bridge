...
- Active sessions, async query operations, and pending interaction state are stored in memory in a single process
- Query-operation state is in memory; a service restart loses active runtime state
- Horizontal scaling and multi-instance coordination are not first-class out of the box yet
- Authentication, authorization, and rate limiting are not built-in public defaults; they should be layered upstream
...
├── config.py                   # Top-level settings bootstrap
├── app/
│   ├── api/
│   │   ├── routes/             # REST endpoints for sessions, queries, health, guardrails, and optional A2A
│   │   ├── services/           # Thin route-facing orchestration for session and query flows
│   │   ├── dependencies.py     # Session manager, tenant context, and optional A2A client wiring
│   │   ├── session_context.py  # Tenant-aware session lookup and wrapper context binding
│   │   ├── error_mapping.py    # Domain-to-HTTP error translation
│   │   ├── errors.py           # Shared API error payload helpers
│   │   └── mcp_capabilities.py # Prompt/resource capability helpers exposed through the API layer
│   ├── core/
│   │   ├── runtime/            # MCP runtime boundary over `mcp-use`
│   │   ├── guardrails/         # Session-scoped guardrail execution, policy, PII, and bias modules
│   │   ├── sessions/           # Session lifecycle plus in-memory stores for operations and interactions
│   │   ├── multimodal/         # Validation, preflight checks, image/PDF shaping, and upload resolution
│   │   ├── session_assets/     # Local temporary asset storage and cleanup for multipart uploads
│   │   ├── clients/            # External integrations such as A2A and bias-detector clients
...
│   ├── models/
│   │   ├── config.py           # Settings and request-time configuration models
│   │   ├── requests.py         # Public request schemas
│   │   └── responses.py        # Public response schemas
│   └── utils/
...
├── docker-compose-dind.yml     # Docker-in-Docker gateway variant
├── docker-compose-dod.yml      # Docker-outside-Docker gateway variant
...
PORT=8000
DEBUG=false
CORS_ORIGINS='["http://localhost","https://localhost","http://127.0.0.1","https://127.0.0.1","http://[::1]","https://[::1]","http://localhost:3000","https://localhost:3000","http://127.0.0.1:3000","https://127.0.0.1:3000","http://[::1]:3000","https://[::1]:3000","http://localhost:5173","https://localhost:5173","http://127.0.0.1:5173","https://127.0.0.1:5173","http://[::1]:5173","https://[::1]:5173","http://localhost:8000","https://localhost:8000","http://127.0.0.1:8000","https://127.0.0.1:8000","http://[::1]:8000","https://[::1]:8000"]'
LOG_LEVEL="INFO"
...
Default CORS is limited to common localhost origins, including HTTPS and IPv6 loopback variants, so the repository does not ship with a fully open cross-origin posture. This does not interfere with the built-in `/docs` UI when you open it from the same service origin.
...
- Temporary multipart assets are cleaned up after sync execution or async operation completion, on session deletion, and by periodic TTL sweeps of local metadata files.
...
- no durable or multi-instance-safe asset backend yet
